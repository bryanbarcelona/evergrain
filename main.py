...
from legacy.orientation import correct_image_orientation
from legacy.enhancements import enhance_image_quality
import os
import logging
import shutil
...
#     # Find scan images
#     scanned_raw_image_list = glob.glob(os.path.join(scans_dir, '*.jpg')) + glob.glob(os.path.join(scans_dir, '*.png'))
...
#                 scan = PhotoSplitter(scan_img, background, dpi=dpi, contrast=contrast, shrink=shrink, deskew=deskew)
#                 for index, photo in enumerate(scan):
#                     output_path = os.path.join(output_dir, f'{str(i).zfill(3)}image-{index}.jpg')
